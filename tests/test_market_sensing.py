...
class MarketSensingTests(unittest.TestCase):
    ...
    def valid_decision_lens(self) -> dict:
        return {
            "schema_version": market_sensing.DECISION_LENS_SCHEMA_VERSION,
            "primary_direction": "mixed",
            "opportunity": {
                "condition": "고객이 새 정책에 따른 저탄소 가격 가산분을 수용합니다.",
                "business_effect": "인증 제품의 장기계약과 단위이익을 함께 높일 수 있습니다.",
                "action": "고객별 구매의향과 검증 배출량을 연결해 물량을 우선 배분합니다.",
            },
            "risk": {
                "condition": "수입쿼터가 먼저 소진되고 고객의 비용 분담이 제한됩니다.",
                "business_effect": "판매 가능 물량과 단위이익이 함께 낮아질 수 있습니다.",
                "action": "고객별 최소 수익성 기준과 대체시장 전환안을 적용합니다.",
            },
            "opportunity_cost": (
                "판단을 늦추면 저탄소 제품의 가격 가산분과 장기계약을 먼저 확보할 "
                "시점을 놓칠 수 있습니다."
            ),
            "decision_trigger": (
                "고객 가격 가산분이 인증비를 넘으면 우선배분을 시작하고, 쿼터가 먼저 "
                "소진되면 방어안으로 전환합니다."
            ),
        }
    ...
    def test_signal_graph_has_four_progressive_depths_and_reader_projection(self):
        ...
        quantification = self.root.parent / "signal-quantification.json"
        market_sensing.write_json(
            quantification,
            {
                "schema_version": 1,
                ...
                "assessed_at": "2026-08-18",
                "omission": {
                    "reason_code": "no_grounded_exposure_basis",
                    "rationale": (
                        "테스트 출처에는 고객별 판매량과 계약 전가율이 없어 현실적인 "
                        "순영향 범위를 계산할 수 없습니다."
                    ),
                    "required_inputs": ["고객별 판매량", "계약별 가격 전가율"],
                    "reconsider_when": (
                        "고객별 판매량과 계약 전가율 중 하나 이상을 확인하면 다시 계산합니다."
                    ),
                },
            },
        )
        ...
        self.assertNotIn("response_deadline", signal["urgency"])
        self.assertEqual(
    ...
    def test_strategic_watch_persists_and_traces_from_warning_to_source(self):
        ...
        self.assertEqual(len(re.findall(r"(?m)^## ", page)), 4)
        self.assertNotIn("## 확인한 원문", page)
        self.assertNotIn("## 기회와 위험을 함께 보는 판단", page)
    ...
    def test_posco_holdings_supports_lithium_and_strategic_minerals_axes(self):
        self.assertTrue(
        ...
        self.assertTrue(
            market_sensing.company_supports_business_axis(
                "COM-POSCO-HOLDINGS", "전략광물"
            )
        )
        self.assertFalse(
            market_sensing.company_supports_business_axis(
                "COM-POSCO-HOLDINGS", "에너지"
            )
        )
        self.assertIn("전략광물", market_sensing.MARKET_SENSING_BUSINESS_AXES)
        self.assertIn("희토류·텅스텐", market_sensing.WIKI_SETTINGS_TEMPLATE)
        self.assertIn("확정 사실로 단정하지 않음", market_sensing.WIKI_SETTINGS_TEMPLATE)
    ...
    def test_signal_analysis_rejects_unquoted_mermaid_parentheses(self):
        analysis = self.valid_signal_analysis() + (
            "\n```mermaid\n"
            "flowchart LR\n"
            " A[시장 변화] --> B[상각 전 영업이익(EBITDA) 영향]\n"
            "```\n"
        )
        with self.assertRaisesRegex(ValueError, "wrapped in double quotes"):
            market_sensing.validate_signal_analysis(analysis)

    def test_signal_analysis_accepts_quoted_mermaid_parentheses(self):
        analysis = self.valid_signal_analysis() + (
            "\n```mermaid\n"
            "flowchart LR\n"
            ' A["시장 변화"] --> B["상각 전 영업이익(EBITDA) 영향"]\n'
    ...
    def test_editorial_migration_applies_schema_v2_and_type_atomically(self):
        ...
        self.assertIn(
            f"schema_version must be {market_sensing.SIGNAL_SCHEMA_VERSION}",
            legacy_report,
        )
    ...
    def test_mkdocs_signal_navigation_groups_axes_and_sorts_newest_first(self):
        mkdocs_config = (PROJECT_TOOLS / "mkdocs.yml").read_text(encoding="utf-8")
        self.assertNotIn("- navigation.sections", mkdocs_config)

        signal_specs = (
            ("SIG-STEEL-OLD", "철강", "2026-08-17", "이전 철강 변화"),
            ("SIG-STEEL-NEW", "철강", "2026-08-19", "최신 철강 변화"),
            ("SIG-LITHIUM", "리튬", "2026-08-18", "리튬 변화"),
            ("SIG-MINERALS", "전략광물", "2026-08-18", "전략광물 변화"),
            ("SIG-ENERGY", "에너지", "2026-08-16", "에너지 변화"),
        )
        for signal_id, axis, assessed_at, title in signal_specs:
            (self.root / "signals" / f"{signal_id}.md").write_text(
                f"# {title}\n",
                encoding="utf-8",
            )
            market_sensing.write_json(
                self.root / ".system" / "signals" / f"{signal_id}.json",
                {"business_axis": axis, "assessed_at": assessed_at},
            )
        ...
        self.assertEqual(signal_nav[0], {"전체 시그널": "signals/index.md"})
        self.assertEqual(
            signal_nav[1:],
            [
                {
                    "철강": [
                        {"최신 철강 변화": "signals/SIG-STEEL-NEW.md"},
                        {"이전 철강 변화": "signals/SIG-STEEL-OLD.md"},
                    ]
                },
                {"리튬": [{"리튬 변화": "signals/SIG-LITHIUM.md"}]},
                {"전략광물": [{"전략광물 변화": "signals/SIG-MINERALS.md"}]},
                {"에너지": [{"에너지 변화": "signals/SIG-ENERGY.md"}]},
            ],
        )
    ...
    def test_signal_list_groups_roles_without_adding_a_role_pill(self):
        ...
        self.assertIn("item.opportunity", script)
        self.assertIn("signal-row-opportunity", script)
        ...
        self.assertIn(".signal-row-opportunity", styles)

    def test_signal_detail_summary_uses_available_content_width(self):
        styles = (
            PROJECT_ROOT
            / "market-sensing-wiki"
            / "stylesheets"
        ...
        self.assertRegex(
            styles,
            r"\.signal-detail-lede\.admonition\s*\{[^}]*max-width:\s*none",
        )
        self.assertRegex(
            styles,
    ...
    def test_financial_impact_simulator_uses_validated_portable_formula(self):
        ...
        self.assertIn("function formatOutput(value, output)", simulator)
        self.assertNotIn('output.unit === "KRW"', simulator)
        self.assertIn('case "multiply"', simulator)
        self.assertIn('range.type = "range"', simulator)
        self.assertIn("dominantVariable", simulator)
        self.assertIn('element("span", "impact-result-label", `${shortLabel} 시나리오`)', simulator)
        self.assertIn('results.classList.add("has-scenarios")', simulator)
        ...
        self.assertIn("repeat(auto-fit, minmax(min(10rem, 100%), 1fr))", styles)
        self.assertIn(".impact-results.has-scenarios", styles)
        self.assertIn(".impact-method > summary::before", styles)
        self.assertIn("font-size: clamp(0.78rem, 7.5cqi, 1.35rem)", styles)
        self.assertIn("grid-column: 1 / -1", styles)
        page = "\n".join(
            market_sensing.signal_page_lines(
                {
                    "sentence": "외부 변화가 사업 판단에 영향을 줍니다.",
                    "business_axis": "철강",
                    "business_impact": {"score": 4, "rationale": "영향 경로"},
                    "urgency": {"score": 3, "rationale": "대응 시점"},
                },
                {
                    "title": "정량화 순서 테스트",
                    "summary": "핵심 해석입니다.",
                    "analysis_markdown": "### 판단 질문과 잠정 결론\n\n상세 분석입니다.",
    ...
    def test_quantification_packet_requires_model_or_reviewable_omission(self):
        with self.assertRaisesRegex(ValueError, "quantification_file is required"):
            market_sensing.resolve_quantification_input(
                Namespace(quantification_file=None, impact_estimate_file=None)
            )
        estimate = json.loads(
            (
                PROJECT_ROOT
        ...
        modeled = market_sensing.modeled_quantification_packet(estimate)
        self.assertEqual(
            market_sensing.validate_quantification_packet(modeled)["decision"],
            "modeled",
        )
        omission = {
            "schema_version": 1,
            ...
            "omission": {
                ...
                "rationale": (
                    "동일한 가격 충격이 다른 Signal 모델에 이미 포함돼 독립 금액으로 "
                    "제시하면 포트폴리오 영향이 중복 합산됩니다."
                ),
                "required_inputs": ["Signal 간 공통 충격 식별자"],
                "reconsider_when": (
                    "공통 충격을 한 번만 합산하는 포트폴리오 모델이 연결되면 재검토합니다."
                ),
            },
        }
        self.assertEqual(
            market_sensing.validate_quantification_packet(omission)["decision"],
            "omitted",
        )
        omission["omission"]["rationale"] = "근거 부족"
        with self.assertRaisesRegex(ValueError, "rationale"):
            market_sensing.validate_quantification_packet(omission)

    def test_quantification_packet_json_schema_is_available_to_api_clients(self):
        schema = json.loads(
            (
                PROJECT_ROOT
                / "skills"
                / "market-sensing-intelligence"
                / "references"
                / "quantification-packet.schema.json"
            ).read_text(encoding="utf-8")
        )
        self.assertEqual(schema["properties"]["schema_version"]["const"], 1)
        self.assertEqual(
            schema["properties"]["decision"]["enum"], ["modeled", "omitted"]
        )

    def test_decision_lens_schema_and_validator_require_both_sides(self):
        schema = json.loads(
            (
                PROJECT_ROOT
                / "skills"
                / "market-sensing-intelligence"
                / "references"
                / "decision-lens.schema.json"
            ).read_text(encoding="utf-8")
        )
        self.assertEqual(schema["properties"]["schema_version"]["const"], 1)
        lens = self.valid_decision_lens()
        self.assertEqual(
            market_sensing.validate_decision_lens(lens)["primary_direction"],
            "mixed",
        )
        del lens["opportunity_cost"]
        with self.assertRaisesRegex(ValueError, "opportunity_cost"):
            market_sensing.validate_decision_lens(lens)
