...
def apply_proposals(proposals: list[dict[str, Any]], wiki_root: Path, backup: Path) -> None:
    ...
    for proposal in proposals:
        ...
        # Editorial copy migration alone cannot claim the balanced schema v3
        # contract. Preserve v3 only when a validated decision lens already exists;
        # otherwise leave the record at v2 for the full lens migration to finish.
        signal["schema_version"] = (
            market_sensing.SIGNAL_SCHEMA_VERSION
            if signal.get("decision_lens")
            else 2
        )
