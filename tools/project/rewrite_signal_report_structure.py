...
4. 판단 질문과 잠정 결론은 첫 H2 아래에서 각각 **판단 질문:**, **잠정 결론:** 라벨로 보존한다.
...
8. 결론 확정·폐기 조건과 판단의 한계 admonition을 보존한다.
9. 모든 수치·날짜·고유명사·인용 취지·표·Mermaid·링크는 그대로 보존한다. 새 사실, 새 수치,
   새 시한, 새 인과관계를 만들지 않는다.
...
13. 출력은 analysis_markdown과 짧은 edit_note만 반환한다.
"""
...
def generate_proposals(wiki_root: Path, output: Path, model: str) -> list[dict[str, Any]]:
    client = OpenAI()
    proposals: list[dict[str, Any]] = []
    insights = sorted(market_sensing.insight_records(wiki_root), key=lambda item: item[0].name)
...
def generate_codex_batch(
    ...
    if not codex_script.is_file() or not node_executable.is_file():
...
def generate_proposals_with_codex(
    wiki_root: Path, output: Path, model: str, batch_size: int
) -> list[dict[str, Any]]:
    insights = sorted(market_sensing.insight_records(wiki_root), key=lambda item: item[0].name)
    proposals: list[dict[str, Any]] = []
    if output.is_file():
...
def apply_proposals(wiki_root: Path, proposal_path: Path, backup_path: Path) -> int:
...
def main() -> int:
    ...
    if args.generate:
        if args.provider == "codex":
            ...
            generate_proposals(args.wiki_root, args.proposals, args.model)
    if args.apply:
        count = apply_proposals(args.wiki_root, args.proposals, args.backup)
