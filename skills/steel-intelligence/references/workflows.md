...
1. 최상위 `WIKI-SETTINGS.md`와 최근 성공 run을 읽는다. 설정을 수정했다면
   `sync-settings`로 JSON 캐시를 갱신한다.
2. 검색 기간을 마지막 성공일보다 3~7일 앞에서 시작해 누락을 줄인다.
3. 기업 공식명·약칭·프로젝트명·현지어·기술 동의어를 조합한다.
4. 공식 출처와 실패 신호를 먼저 검색한다.
5. 후보마다 본문, 게시일, 발행자, 원 URL을 확인한다. 설비 형태·공정 구성을
   이해하는 데 필요하면 원문 이미지와 캡션·권리 조건도 확인한다.
6. 쿼리와 결과를 run JSON에 기록한다.
7. 후보를 ingest로 넘긴다.
...
5. 등록된 `.system/raw/` 파일을 수정하지 않는다.

이미지가 필요한 Source는 본문 등록 후 한 장씩 연결한다. 여러 장이면 `add-image`를
반복한다. 사용 조건이 확인된 로컬 파일 또는 공개 이미지 URL은 다음처럼 등록한다.

```powershell
python skills/steel-intelligence/scripts/steel_intel.py add-image steel-wiki `
...
```powershell
python skills/steel-intelligence/scripts/steel_intel.py add-claim steel-wiki `
  --subject-id PRJ-HAMBURG-DRI `
...
```powershell
python skills/steel-intelligence/scripts/steel_intel.py render-report steel-wiki `
  --input .\steel-wiki\reports\briefs\custom-report.md
```
...
```powershell
python skills/steel-intelligence/scripts/steel_intel.py search steel-wiki `
...
Obsidian에서는 `steel-wiki`를 Vault로 연다. 자동 투영본이 없거나 JSON을 외부에서
수정했다면 다음 명령으로 전체 링크를 다시 생성한다.

```powershell
python skills/steel-intelligence/scripts/steel_intel.py sync-obsidian steel-wiki
```

이 명령은 MkDocs 시작 화면의 기술×기업 매트릭스와 Obsidian Markdown을 함께 갱신한다.
