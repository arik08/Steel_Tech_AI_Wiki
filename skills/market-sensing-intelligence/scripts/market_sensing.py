...
MARKET_SENSING_AXES = {
    "COM-POSCO": frozenset({"철강"}),
    "COM-POSCO-HOLDINGS": frozenset({"리튬", "전략광물"}),
    "COM-POSCO-INTERNATIONAL": frozenset({"에너지"}),
}
MARKET_SENSING_BUSINESS_AXES = frozenset(
    axis for axes in MARKET_SENSING_AXES.values() for axis in axes
)


def company_supports_business_axis(company_id: str, business_axis: str) -> bool:
    return business_axis in MARKET_SENSING_AXES.get(company_id, frozenset())
...
- 포스코홀딩스가 국가안보·경제안보 관점에서 검토할 전략광물 신사업의 신규 진입 조건을 바꾸는 외부 변화
- 전략광물 대상 원소는 미확정 후보군으로 관리하고, 희토류·텅스텐 등을 포함하되 정부의 공식 지정·요청 범위가 확인되기 전에는 특정 원소나 회장 지시 내용을 확정 사실로 단정하지 않음
...
- 모든 Signal과 핵심 전략 이슈에 기회 조건·사업 효과·선제 행동, 위험 조건·사업 효과·
  방어 행동, 미실행 기회비용, 결정 전환 조건을 함께 저장하세요. 대표 방향과 무관하게
  `decision-lens.schema.json` 및 audit로 양쪽 분석을 검증하세요.
...
> 마지막 기준일 이후 새로 확인되거나 달라진 사업축에 영향을 주는 외부 변화만
> 모아 보는 변화 중심 브리프입니다.
...
def validate_signal_analysis(markdown: str) -> None:
    ...
    required_concepts = {
        ...
        "반증 조건": ("반증 조건", "결론 확정·폐기 조건", "결론을 확정·폐기"),
        "확인 담당": ("확인 담당",),
        "감지 트리거": ("감지 트리거",),
    }
    missing = [
    ...
    for block in re.finditer(
        ...
        for node in re.finditer(r"\b[A-Za-z_][\w-]*\[([^\]\r\n]+)\]", block.group(1)):
            label = node.group(1).strip()
            if (
                ...
                raise ValueError(
                    "Mermaid node labels containing parentheses must be wrapped "
                    "in double quotes"
                )
...
def validate_decision_lens(value: Any, label: str = "decision_lens") -> dict[str, Any]:
    """Require a balanced opportunity, risk, and cost-of-inaction assessment."""
    if not isinstance(value, dict):
        raise ValueError(f"{label} must be an object")
    if value.get("schema_version") != DECISION_LENS_SCHEMA_VERSION:
        raise ValueError(
            f"{label}.schema_version must be {DECISION_LENS_SCHEMA_VERSION}"
        )
    direction = str(value.get("primary_direction") or "").strip()
    if direction not in DECISION_LENS_DIRECTIONS:
        raise ValueError(
            f"{label}.primary_direction must be one of: "
            + ", ".join(sorted(DECISION_LENS_DIRECTIONS))
        )
    for side in ("opportunity", "risk"):
        assessment = value.get(side)
        if not isinstance(assessment, dict):
            raise ValueError(f"{label}.{side} must be an object")
        for field in ("condition", "business_effect", "action"):
            text = re.sub(r"\s+", " ", str(assessment.get(field) or "")).strip()
            if not 10 <= len(text) <= 300:
                raise ValueError(f"{label}.{side}.{field} must be 10-300 characters")
    for field in ("opportunity_cost", "decision_trigger"):
        text = re.sub(r"\s+", " ", str(value.get(field) or "")).strip()
        if not 20 <= len(text) <= 500:
            raise ValueError(f"{label}.{field} must be 20-500 characters")
    return value


def read_decision_lens(path_value: str | None) -> dict[str, Any]:
    if not path_value:
        raise ValueError(
            "decision_lens_file is required: assess opportunity, risk, "
            "opportunity cost, and the decision trigger"
        )
...
QUANTIFICATION_OMISSION_REASONS = {
    "no_grounded_exposure_basis",
    "non_financial_decision_signal",
...
def validate_quantification_packet(value: Any) -> dict[str, Any]:
    """Require either a portable model or an explicit, reviewable omission."""
    if not isinstance(value, dict):
        raise ValueError("quantification packet must be a JSON object")
    if value.get("schema_version") != QUANTIFICATION_SCHEMA_VERSION:
        raise ValueError("quantification packet schema_version must be 1")
    decision = str(value.get("decision") or "")
    if decision not in QUANTIFICATION_DECISIONS:
        raise ValueError("quantification decision must be modeled or omitted")
    assessed_at = validate_date(
        str(value.get("assessed_at") or ""), "quantification assessed_at"
    )
    if decision == "modeled":
        estimate = validate_impact_estimate(value.get("impact_estimate"))
        if assessed_at != estimate["as_of"]:
            raise ValueError(
                "modeled quantification assessed_at must equal impact estimate as_of"
            )
        if value.get("omission") is not None:
            raise ValueError("modeled quantification must not contain omission")
        return value

    if value.get("impact_estimate") is not None:
        raise ValueError("omitted quantification must not contain impact_estimate")
    omission = value.get("omission")
    if not isinstance(omission, dict):
        raise ValueError("omitted quantification requires an omission object")
    if omission.get("reason_code") not in QUANTIFICATION_OMISSION_REASONS:
        raise ValueError(
            "quantification omission reason_code must be one of: "
            + ", ".join(sorted(QUANTIFICATION_OMISSION_REASONS))
        )
    rationale = re.sub(r"\s+", " ", str(omission.get("rationale") or "")).strip()
    if not 40 <= len(rationale) <= 500:
        raise ValueError("quantification omission rationale must be 40-500 characters")
    required_inputs = omission.get("required_inputs")
    if not isinstance(required_inputs, list) or not 1 <= len(required_inputs) <= 6:
        raise ValueError("quantification omission requires 1-6 required_inputs")
    if any(
        not isinstance(item, str) or not 5 <= len(item.strip()) <= 160
        for item in required_inputs
    ):
        raise ValueError(
            "quantification omission required_inputs must be 5-160 character strings"
        )
    reconsider_when = re.sub(
        r"\s+", " ", str(omission.get("reconsider_when") or "")
    ).strip()
    if not 20 <= len(reconsider_when) <= 300:
        raise ValueError(
            "quantification omission reconsider_when must be 20-300 characters"
        )
    return value


def read_quantification_packet(path_value: str | None) -> dict[str, Any] | None:
    if not path_value:
        return None
    return validate_quantification_packet(read_json(Path(path_value)))


def modeled_quantification_packet(estimate: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": QUANTIFICATION_SCHEMA_VERSION,
        "decision": "modeled",
...
def resolve_quantification_input(args: argparse.Namespace) -> dict[str, Any]:
    packet = read_quantification_packet(
        getattr(args, "quantification_file", None)
    )
    estimate = read_impact_estimate(getattr(args, "impact_estimate_file", None))
    if packet is not None and estimate is not None:
        raise ValueError(
            "use quantification_file or impact_estimate_file, not both"
        )
    if packet is None and estimate is not None:
        packet = modeled_quantification_packet(estimate)
    if packet is None:
        raise ValueError(
            "quantification_file is required: provide a grounded model or an "
            "explicit omission decision"
        )
    return packet
...
def add_signal(args: argparse.Namespace) -> dict[str, Any]:
    ...
    referenced_model_sources = {
        str(source_id)
        for variable in (impact_estimate or {}).get("variables", [])
        for source_id in variable.get("source_ids", [])
    }
    unlinked_model_sources = sorted(referenced_model_sources - set(source_ids))
    if unlinked_model_sources:
        raise ValueError(
            "impact model sources must be linked through the Signal claims: "
            + ", ".join(unlinked_model_sources)
        )
    ...
    signal = {
        ...
        "urgency": {
            ...
            **({"response_deadline": response_deadline} if response_deadline else {}),
        },
        "assessed_at": assessed_at,
        "assessment_confidence": args.assessment_confidence,
        "claim_ids": claim_ids,
...
def set_impact_estimate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    insight["quantification"] = modeled_quantification_packet(estimate)
...
def set_quantification(args: argparse.Namespace) -> dict[str, Any]:
    """Attach a complete model-or-omission decision to an existing Signal."""
    root = require_store(Path(args.root))
    signals = _records_by_id(signal_records(root), "signal_id")
    signal = signals.get(args.signal_id)
    if signal is None:
    ...
    if insight_id not in insight_paths:
        raise ValueError(f"Broken insight link: {insight_id}")
    packet = read_quantification_packet(args.quantification_file)
    if packet is None:
        raise ValueError("quantification_file is required")
    estimate = packet.get("impact_estimate")
    referenced_sources = {
        str(source_id)
        for variable in (estimate or {}).get("variables", [])
        for source_id in variable.get("source_ids", [])
    }
    verify_source_ids(root, sorted(referenced_sources))
    insight_path, insight = insight_paths[insight_id]
    insight["quantification"] = packet
    insight["impact_estimate"] = estimate
    insight["updated_at"] = timestamp()
    write_json(insight_path, insight)
    sync_obsidian_store(root)
    append_log(
        root,
        "set-quantification",
        f"{args.signal_id}: {packet['decision']}",
    )
    return {
        "action": "quantification_updated",
...
def apply_quantification_manifest(args: argparse.Namespace) -> dict[str, Any]:
    """Atomically migrate existing models and apply reviewed batch decisions."""
    root = require_store(Path(args.root))
    manifest = read_json(Path(args.manifest_file))
    if manifest.get("schema_version") != QUANTIFICATION_SCHEMA_VERSION:
        raise ValueError("quantification manifest schema_version must be 1")
    decisions = manifest.get("decisions")
    if not isinstance(decisions, dict):
        raise ValueError("quantification manifest decisions must be an object")
    signals = _records_by_id(signal_records(root), "signal_id")
    unknown_signals = sorted(set(decisions) - set(signals))
    if unknown_signals:
        raise ValueError(
            "quantification manifest has unknown Signals: "
    ...
    source_ids = {source_id for source_id in _records_by_id(source_records(root), "source_id")}
    updates: list[tuple[Path, dict[str, Any], str]] = []
    missing: list[str] = []
    counts = {"modeled": 0, "omitted": 0}
    for signal_id, signal in sorted(signals.items()):
        insight_id = str(signal.get("insight_id") or "")
        if insight_id not in insight_paths:
            raise ValueError(f"Broken insight link for {signal_id}: {insight_id}")
        insight_path, insight = insight_paths[insight_id]
        packet_value = decisions.get(signal_id)
        if packet_value is None and insight.get("quantification") is not None:
            packet_value = insight["quantification"]
        if packet_value is None and insight.get("impact_estimate") is not None:
            packet_value = modeled_quantification_packet(insight["impact_estimate"])
        if packet_value is None:
            missing.append(signal_id)
            continue
        packet = validate_quantification_packet(packet_value)
        estimate = packet.get("impact_estimate")
        referenced_sources = {
            str(source_id)
            for variable in (estimate or {}).get("variables", [])
        ...
        if unknown_sources:
            raise ValueError(
                f"{signal_id} impact model has unknown sources: "
        ...
        if unlinked_sources:
            raise ValueError(
                f"{signal_id} impact model sources are not linked through claims: "
                + ", ".join(unlinked_sources)
            )
        updated = dict(insight)
        updated["quantification"] = packet
        updated["impact_estimate"] = estimate
        updated["updated_at"] = timestamp()
        updates.append((insight_path, updated, signal_id))
        counts[packet["decision"]] += 1
    if missing:
        raise ValueError(
            "quantification manifest is incomplete; missing decisions for: "
            + ", ".join(missing)
        )
    for insight_path, insight, _ in updates:
        write_json(insight_path, insight)
    sync_obsidian_store(root)
    append_log(
        root,
        "apply-quantification-manifest",
        f"{counts['modeled']} modeled, {counts['omitted']} omitted",
    )
    return {
        "action": "quantification_manifest_applied",
        "modeled": counts["modeled"],
...
def decision_lens_block_lines(lens: dict[str, Any] | None) -> list[str]:
    ...
    return [
        ...
        '!!! success "포착할 기회"',
        "",
        f"    **조건:** {markdown_cell(opportunity.get('condition') or '-')}",
        "",
        f"    **사업 효과:** {markdown_cell(opportunity.get('business_effect') or '-')}",
        "",
        f"    **선제 행동:** {markdown_cell(opportunity.get('action') or '-')}",
        "",
        '!!! danger "방어할 위험"',
        "",
        f"    **조건:** {markdown_cell(risk.get('condition') or '-')}",
        "",
        f"    **사업 효과:** {markdown_cell(risk.get('business_effect') or '-')}",
        "",
        f"    **방어 행동:** {markdown_cell(risk.get('action') or '-')}",
        "",
        '!!! warning "미실행 기회비용"',
        "",
...
def decision_lens_value(record: dict[str, Any], side: str, field: str) -> str:
    lens = record.get("decision_lens") or {}
    side_record = lens.get(side) or {}
    return str(side_record.get(field) or "").strip()
...
def signal_index_lines(
    ...
    for signal in ordered:
        ...
        opportunity = markdown_cell(
            decision_lens_value(signal, "opportunity", "business_effect")
            or "기회 효과 미정"
        )
        ...
        lines.append(
            ...
            f"| {wikilink(signal_path, sentence)}<br>**기회 ·** {opportunity} "
            f"| {markdown_cell(signal.get('assessed_at') or '-')} |"
        )
    if not ordered:
...
def _strategic_indicator_lines(
    trends: list[dict[str, Any]], sources_by_id: dict[str, dict[str, Any]]
) -> list[str]:
    indicators = [
        indicator
        for trend in trends
    ...
    for indicator in indicators:
        evidence_links = []
        for index, source_id in enumerate(indicator.get("source_ids", []), 1):
            source = sources_by_id.get(str(source_id))
            if not source:
                continue
            evidence_links.append(
                wikilink(Path("sources") / f"{source_id}.md", f"원문 {index}")
            )
        lines.append(
            f"| {markdown_cell(indicator.get('label') or '-')} "
            f"| **{markdown_cell(_indicator_display_value(indicator))}** "
            f"| {markdown_cell(indicator.get('observed_at') or '-')} "
            f"| {markdown_cell(indicator.get('interpretation') or '-')} "
            f"| {' · '.join(evidence_links) or '-'} |"
        )
    lines.append("")
    return lines


def _strategic_decision_lens_lines(lens: dict[str, Any] | None) -> list[str]:
    if not isinstance(lens, dict):
        return []
    opportunity = lens.get("opportunity") or {}
    risk = lens.get("risk") or {}
...
def strategic_warning_page_lines(
    ...
    business_impact = section_by_role.get("business_impact") or {}
    recommendation = section_by_role.get("recommendation") or {}
    evidence = section_by_role.get("evidence") or {}
    monitoring = section_by_role.get("monitoring") or {}
    limitations = section_by_role.get("limitations") or {}

    lines.extend(
        [
            f"## {market_change.get('heading') or '시장 변화'}",
            "",
            str(market_change.get("body") or "-").strip(),
            "",
            f"**{markdown_cell(assumption_shift.get('heading') or '바뀐 전제')}.**",
            "",
    ...
    lines.extend(
        [
            f"**판단 범위:** {markdown_cell(thesis.get('decision_horizon') or '-')} · "
            f"분석 신뢰도 {THESIS_CONFIDENCE_LABELS.get(str(thesis.get('confidence')), '-')}",
            "",
            f"## {recommendation.get('heading') or '권고 대응'}",
            "",
    ...
    lines.extend(
        [
            "",
            f"**{markdown_cell(monitoring.get('heading') or '후속 확인선')}.**",
            "",
    ...
    lines.extend(
        [
            "",
            f"## {evidence.get('heading') or '판단 근거'}",
            "",
            str(evidence.get("body") or "-").strip(),
            "",
            "**연결된 마켓 시그널**",
            "",
        ]
    ...
    lines.extend(["", f'??? note "{markdown_cell(limitations.get("heading") or "분석의 한계")}"', ""])
    for body_line in str(limitations.get("body") or "-").strip().splitlines():
...
def strategic_warning_index_lines(warnings: list[dict[str, Any]]) -> list[str]:
    ...
    for warning in active:
        opportunity = markdown_cell(
            decision_lens_value(warning, "opportunity", "business_effect")
            or "기회 효과 미정"
        )
        lines.append(
...
def sync_obsidian_store(root: Path) -> dict[str, Any]:
    ...
    index_lines = [
        ...
        "# 포스코그룹 마켓센싱",
        "",
        "철강·리튬·전략광물·에너지 사업의 의사결정에 영향을 줄 외부 변화를 선별해 "
    ...
    for warning in active_warnings:
        ...
        index_lines.extend(
            [
                ...
                f'{markdown_cell(warning.get("business_axis") or "-")} · '
                f'{STRATEGIC_ISSUE_DIRECTION_LABELS.get(str(warning.get("issue_direction")), "방향 미정")}"',
                "",
...
def audit_store(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for path, signal in signal_records(root):
        ...
        try:
            validate_decision_lens(
                signal.get("decision_lens"), f"{signal_id}.decision_lens"
            )
        except ValueError as exc:
        ...
        quantification = insight.get("quantification")
        try:
            validate_quantification_packet(quantification)
        except ValueError as exc:
            findings["signal_quality"].append(
                f"{signal_id}: invalid or missing quantification decision: {exc}"
            )
        else:
            packet_estimate = quantification.get("impact_estimate")
            if packet_estimate != impact_estimate:
                findings["signal_integrity"].append(
                    f"{signal_id}: quantification model and impact_estimate differ"
                )
            referenced_model_sources = {
                str(source_id)
                for variable in (packet_estimate or {}).get("variables", [])
                for source_id in variable.get("source_ids", [])
            }
            unknown_model_sources = sorted(referenced_model_sources - source_ids)
            if unknown_model_sources:
                findings["signal_integrity"].append(
                    f"{signal_id}: impact model references unknown sources "
                    + ", ".join(unknown_model_sources)
                )
            unlinked_model_sources = sorted(
                referenced_model_sources
                - {str(item) for item in insight.get("source_ids", [])}
            )
            if unlinked_model_sources:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    signal_parser.add_argument(
        "--decision-lens-file", required=True,
        help=(
    ...
    signal_parser.add_argument(
        ...
        help="Legacy shortcut for a modeled quantification packet.",
    )
    signal_parser.add_argument(
        "--quantification-file",
        help=(
            "Required model-or-omission JSON decision. Every Signal must provide this "
            "or the legacy impact-estimate-file shortcut."
        ),
    )
    ...
    signal_parser.add_argument(
        "--response-deadline",
        help=(
            "Optional ISO date. Use only for a real, evidence-backed legal, application, "
            "contractual, or official decision deadline; omit when uncertain."
        ),
    )
    ...
    quantification_parser = subparsers.add_parser(
        "set-quantification",
        help="Attach a validated model-or-explicit-omission decision to a Signal.",
    )
    quantification_parser.add_argument("root")
    quantification_parser.add_argument("--signal-id", required=True)
    quantification_parser.add_argument("--quantification-file", required=True)
    quantification_parser.set_defaults(func=set_quantification)

    quantification_manifest_parser = subparsers.add_parser(
        "apply-quantification-manifest",
        help=(
            "Atomically migrate existing impact models and apply model-or-omission "
            "decisions for every Signal."
        ),
    )
    quantification_manifest_parser.add_argument("root")
    quantification_manifest_parser.add_argument("--manifest-file", required=True)
    quantification_manifest_parser.set_defaults(func=apply_quantification_manifest)
