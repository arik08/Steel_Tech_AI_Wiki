# Steel Intelligence LLM Wiki

해외 경쟁사의 철강 신기술을 검색하고, 출처 원문과 검증 가능한 주장을 누적하며,
변경·충돌·취소 정보를 사람 검토로 관리하는 Codex용 로컬 스킬입니다.
...
- `관심사 설정 기준으로 최근 해외 철강 신기술을 조사해서 누적해줘.`
- `SSAB 수소환원제철 프로젝트의 일정 변경을 기존 자료와 비교해줘.`
- `저장된 자료에서 유럽 DRI 프로젝트의 가동 목표를 찾아 출처와 함께 답해줘.`
- `지난 보고 이후 변경사항을 Markdown과 HTML 보고서로 만들어줘.`
- `중복 출처와 충돌 Claim을 감사해줘.`
- `설비 형태를 이해하는 데 필요한 공식 사진이나 공정도를 출처와 함께 넣어줘.`

최상위 `AGENTS.md`가 Codex에 `skills/steel-intelligence/SKILL.md`를 읽고 따르도록
...
4. 좌측 탐색과 문서 목차를 사용하고 본문의 출처 링크를 누르면 근거 문서로
   바로 이동합니다.
5. 조사 운영자는 `steel-wiki/index.md`와 `REVIEW.md`에서 전체 현황과 검토 대기
   항목을 확인합니다.
6. Obsidian을 쓰는 경우 `steel-wiki`를 Vault로 열어 Markdown 위키링크와 그래프를
   함께 사용합니다.
...
| 경로 | 역할 | 주의 사항 |
|---|---|---|
| `steel-wiki/AGENTS.md` | 지식 저장소 내부에서 지켜야 할 불변 규칙 | 원문 불변·Claim 이력 보존 |
| `steel-wiki/index.md` | 현황·관심사·최근 변경·회사·기술·프로젝트와 기술×기업 매트릭스를 모은 시작 화면 | 자동 생성, 직접 수정 금지 |
| `steel-wiki/REVIEW.md` | 사람이 판단해야 할 충돌·중복 검토 대기열 | 결론은 Codex에 전달 |
| `steel-wiki/log.md` | 등록·검토·감사·보고서 작업 이력 | 도구가 추가 기록 |
...
다음 `steel_intel.py` 명령이 실행될 때 자동으로 JSON에 반영되며, 즉시 반영하려면
...
```powershell
python skills/steel-intelligence/scripts/steel_intel.py sync-settings steel-wiki
```

현재 적용된 설정 확인:

```powershell
python skills/steel-intelligence/scripts/steel_intel.py show-settings steel-wiki
```
...
```powershell
python skills/steel-intelligence/scripts/steel_intel.py audit steel-wiki
```

기준일 이후 변경 브리프와 HTML 생성:

```powershell
python skills/steel-intelligence/scripts/steel_intel.py brief steel-wiki `
  --since 2026-01-01 `
  --html
```
...
`skills/steel-intelligence/references/workflows.md`를 참고합니다.
...
1. Obsidian에서 `steel-wiki` 폴더를 Vault로 엽니다.
2. `index.md`에서 회사·기술·프로젝트·출처를 탐색합니다.
...
누적된 조사 내용까지 공유하려면 `steel-wiki`를 그대로 포함하고, 빈 템플릿만
