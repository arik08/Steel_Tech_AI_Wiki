...
```mermaid
flowchart TB
    A["외부 변화 · 수소환원제철 경쟁이 고품위 펠렛 확보로 확장"]
    B["전제 변화 · 청정수소 가격이 기술 승자를 결정한다는 단일변수 접근의 한계"]
    C["사업 영향 · 원료 유연성이 HyREX의 원가·공급안정·기술수출 가치를 동시 확대"]
    D["판단 · HyREX 실증의 성공기준을 광종별 총현금원가로 확장"]
    A --> B --> C --> D
```
