...
| 지표 | 현재 확인값 | 기준일 | 해석 | 근거 |
| --- | ---: | --- | --- | --- |
| EU 무관세 철강 수입한도 | **18.3 Mt/년** | 2026-06-30 | 2024년 쿼터보다 47% 줄어 쿼터 1톤의 가치가 높아집니다. | [[sources/SRC-20260818-5C2CEB19|원문 1]] |
...
```mermaid
flowchart TB
    A["외부 변화 · EU 철강 수입축소와 저탄소 수요창출이 결합"]
    B["전제 변화 · 국가·품목별 쿼터 소진율만 관리하던 방식의 한계"]
    C["사업 영향 · 제품 믹스·가격협상·저탄소 인증투자가 하나로 연결되는 손익 경로"]
    D["판단 · 2027년 쿼터를 고객별 순마진과 조달자격으로 재배분"]
    A --> B --> C --> D
```
...
| --- | --- | --- |
| **기회** | 저탄소 조달기준이 실제 고객 구매와 가격 가산분으로 이어지고 검증 배출량이 경쟁재보다 우위 | 희소한 수입쿼터를 고마진·저탄소 제품에 배분해 쿼터 1톤당 순이익과 장기계약 가치를 높임 고객별 순마진·탄소요건·구매의향을 결합한 쿼터 배분표로 물량을 선제 재배치 |
| **위험** | 쿼터는 조기 소진되지만 저탄소 기준 입법과 고객 가격 가산분이 지연 | 수출 가능 물량과 단위이익이 함께 줄고 인증·탄소자료 비용만 먼저 발생 고객별 최소 수익성 기준을 적용하고 현지 생산·대체시장 전환 손익을 병행 비교 |
...
**권고 시한:** 2026-12-31 · **담당:** 포스코 글로벌마케팅·통상·저탄소전략
