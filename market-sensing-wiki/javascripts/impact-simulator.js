(() => {
  ...
  function buildSimulator(model) {
    ...
    for (const output of model.outputs) {
      ...
      results.append(item);
      outputValues.set(output.id, { value, unit });
    }
    if (model.outputs.length === 1 && model.presets.length >= 2) {
      results.classList.add("has-scenarios");
      const output = primaryOutput(model);
      const defaultValues = { ...values };
      for (const preset of model.presets) {
        const shortLabel = String(preset.label).replace(/\s*시나리오$/u, "");
        const scenarioValues = { ...defaultValues, ...preset.values };
        const formatted = formatOutput(
          evaluate(output.expression, scenarioValues),
          output,
        );
        const item = element("div", "impact-result is-scenario");
        item.append(
          element("span", "impact-result-label", `${shortLabel} 시나리오`),
          element("strong", "impact-result-value", formatted.value),
          element("span", "impact-result-unit", formatted.unit),
        );
        results.append(item);
      }
    }
    ...
    function update() {
      try {
        for (const output of model.outputs) {
          ...
          const nodes = outputValues.get(output.id);
          nodes.value.textContent = formatted.value;
          nodes.unit.textContent = formatted.unit;
        }
